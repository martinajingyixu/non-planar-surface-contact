function vectarrow(p0,p1,color,linewidth)
...
if nargin<3
  ...
  if max(size(p0))==3
      if max(size(p1))==3
          ...
          y1 = p1(2);
          z1 = p1(3);
          plot3([x0;x1],[y0;y1],[z0;z1],'color',color,'LineWidth',linewidth);   % Draw a line between p0 and p1
...
%           hw = [z1-alpha*(p(3)+beta*(p(1)+eps));z1;z1-alpha*(p(3)-beta*(p(1)+eps))];
          ...
          hold on
          plot3(hu(:),hv(:),hw(:),'color',color,'LineWidth',linewidth)  % Plot arrow head
          grid on
          xlabel('x')
          ylabel('y')
          zlabel('z')
